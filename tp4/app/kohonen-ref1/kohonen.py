...
def init_kohonen_grid(random_init: bool, data: np.ndarray) :
    # w0 function
    if not random_init:
        w0 = w0_data_init
    else:
...
def get_best_neuron_indexes(grid, data: np.ndarray):
...
def get_neighbour_neuron_indexes(grid_side: int, best, radius: float) :
    neighbours = []
...
# read config file
with open("tp4/app/kohonen-ref1/config.json") as file:
...
# initialize grid
kohonen_grid = init_kohonen_grid(is_w0_random_init, data_scaled)
...
for iteration in range(0, iterations):
    ...
    for _data in data_scaled:
        # get the best neuron position
        pos_best = get_best_neuron_indexes(kohonen_grid, _data)

        # get the best neuron's neighbours indexes
        pos_neighbours = get_neighbour_neuron_indexes(k, pos_best, r)
...
country_list_grid = [[[] for x in range(k)] for x in range(k)]
registry_count_grid = np.zeros((k, k), dtype=int)
...
# U matrix 
u_matrix_grid = np.zeros((k, k), dtype=float)
for i, row in enumerate(kohonen_grid):
    for j, n in enumerate(row):
        # get neighbours index list
        neighbours = get_neighbour_neuron_indexes(k, (i, j), r)
