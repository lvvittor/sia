...
def train_linear_cross_validation(inputs, expected_outputs):
    """
    ...
    for i in range(fold_amount):
        ...
        epochs, _, = linear_perceptron.train(settings.linear_perceptron.epochs)
        test_output = linear_perceptron.predict(test_input)
        errors = abs((test_expected_output - test_output)/test_expected_output)
        corrects = np.count_nonzero(errors < 0.05)
        linear_error = linear_perceptron.get_error()
        print(f"linear error is {linear_error}")
        ...
        if amount_correct == -1 or amount_correct < corrects or (amount_correct == corrects and linear_error < best_error ):
            best_fold = i
            best_error = linear_error
...
def train_multilayer_cross_validation(inputs, expected_outputs):
    """
        Divide input array in N pieces, where 1 piece is for testing, N-1 pieces is for training
        Test with each piece and check which piece trained better.
    """
    ...
    for i in range(fold_amount):
        ...
        if amount_correct == -1 or amount_correct < corrects or (amount_correct == corrects and multilayer_error < best_error ):
            best_fold = i
            best_error = multilayer_error
            amount_correct = corrects
            best_input = train_input
            best_input_output = train_expected_output
            best_test = test_input
