...
def exercise_1():
    ...
    if settings.middle_point.execute:
        ...
        _, _, _, p = autoencoder.encoder.feed_forward(
            np.array(
                [
                    inputs[settings.middle_point.first_input_index],
                    inputs[settings.middle_point.second_input_index],
                ]
            )
        )
        ...
        _, _, _, decoded_p = autoencoder.decoder.feed_forward(
            np.array([p[0], p[1], middle_point])
        )
        ...
        titles = [
            f"Character {settings.middle_point.first_input_index + 1}",
            f"Character {settings.middle_point.second_input_index + 1}",
            f"Middle point",
        ]
        visualize_characters(
            characters,
            suptitle="Middle point",
            titles=titles,
            filename=f"middle-point-{settings.middle_point.first_input_index}-{settings.middle_point.second_input_index}",
        )
...
def _denoising_autoencoder(original_inputs: np.array) -> None:
    ...
    original_inputs_repeated = np.repeat(
        original_inputs, settings.denoising_autoencoder.data_augmentation_factor, axis=0
    )
    noisy_inputs = add_noise(
        original_inputs_repeated, noise_level=settings.denoising_autoencoder.train_noise
    )
    autoencoder = Autoencoder(
        inputs=noisy_inputs,
        hidden_nodes=[16],
        latent_dim=7,
        expected_output=original_inputs_repeated,
    )
    autoencoder.train(settings.epochs)
    ...
    for iteration in range(settings.denoising_autoencoder.predict_rounds):
        ...
        for noise in settings.denoising_autoencoder.predict_noises:
            ...
            noise_iteration_binary_cross_entropies.append(
                binary_cross_entropy(original_inputs, predicted_denoised_inputs)
            )
            noise_iteration_different_pixels.append(
                np.mean(np.sum(original_inputs != predicted_denoised_inputs, axis=1))
            )
            ...
            if iteration == settings.denoising_autoencoder.predict_rounds - 1:
                for i, (
                    original_input,
                    noisy_input,
                    predicted_denoised_input,
                ) in enumerate(
                    zip(original_inputs, noisy_inputs, predicted_denoised_inputs)
                ):
                    if i < 5:  # Only visualize the first 5 characters
                        ...
                        visualize_characters(
                            characters,
                            titles,
                            filename=f"iteration-{iteration}-noise-{noise}-character-{i}",
                        )

        binary_cross_entropies_by_noise_level.append(
            noise_iteration_binary_cross_entropies
        )
        different_pixels_by_noise_level.append(noise_iteration_different_pixels)
    ...
    ax.set(
        ...
        title="Binary Cross Entropy between the original and the denoised outputs",
    )

    plt.xticks(
        range(len(settings.denoising_autoencoder.predict_noises)),
        settings.denoising_autoencoder.predict_noises,
    )
    ...
    ax.set(
        xlabel="Noise level",
        ylabel="Average number of wrong pixels",
        title="Average number of wrong pixels between the original and the denoised outputs",
    )
    ...
    plt.savefig(settings.Config.output_path + "/avg-wrong-pixels.png")
    plt.show()
    plt.clf()
